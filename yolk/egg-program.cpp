...
void egg::yolk::EggProgramSymbolTable::addBuiltins() {
  ...
  this->addBuiltin("string", egg::ovum::ValueFactory::createBuiltinString(this->allocator));
  this->addBuiltin("type", egg::ovum::ValueFactory::createBuiltinType(this->allocator));
  this->addBuiltin("assert", egg::ovum::ValueFactory::createBuiltinAssert(this->allocator));
  this->addBuiltin("print", egg::ovum::ValueFactory::createBuiltinPrint(this->allocator));
}
...
#if defined(WIBBLE)
egg::ovum::Value egg::yolk::EggProgramContext::set(const egg::ovum::String& name, const egg::ovum::Value& rvalue) {
...
egg::ovum::Value egg::yolk::EggProgramContext::guard(const egg::ovum::String& name, const egg::ovum::Value& rvalue) {
  if (rvalue.hasFlowControl()) {
    return rvalue;
  }
  auto symbol = this->symtable->findSymbol(name);
  ...
  if (retval->getVoid()) {
    ...
    return egg::ovum::Value::True;
  }
  return egg::ovum::Value::False;
}

egg::ovum::Value egg::yolk::EggProgramContext::assign(EggProgramAssign op, const IEggProgramNode& lhs, const IEggProgramNode& rhs) {
...
egg::ovum::Value egg::yolk::EggProgramContext::mutate(EggProgramMutate op, const IEggProgramNode& lvalue) {
...
egg::ovum::Value egg::yolk::EggProgramContext::condition(const IEggProgramNode& expression) {
...
egg::ovum::Value egg::yolk::EggProgramContext::unary(EggProgramUnary op, const IEggProgramNode& expr, egg::ovum::Value& value) {
  switch (op) {
  case EggProgramUnary::LogicalNot:
    if (this->operand(value, expr, egg::ovum::ValueFlags::Bool, "Expected operand of logical-not '!' operator to be 'bool'")) {
      return egg::ovum::Value(!value.getBool());
    }
    return value;
  case EggProgramUnary::Negate:
    if (this->operand(value, expr, egg::ovum::ValueFlags::Arithmetic, "Expected operand of negation '-' operator to be 'int' or 'float'")) {
      return value.isInt() ? egg::ovum::Value(-value.getInt()) : egg::ovum::Value(-value.getFloat());
    }
    return value;
  case EggProgramUnary::BitwiseNot:
    if (this->operand(value, expr, egg::ovum::ValueFlags::Int, "Expected operand of bitwise-not '~' operator to be 'int'")) {
      return egg::ovum::Value(~value.getInt());
    }
    return value;
  case EggProgramUnary::Ref:
    ...
    return egg::ovum::ValueFactory::createPointer(this->allocator, value);
...
egg::ovum::Value egg::yolk::EggProgramContext::binary(EggProgramBinary op, const IEggProgramNode& lhs, const IEggProgramNode& rhs, egg::ovum::Value& left, egg::ovum::Value& right) {
  // OPTIMIZE
  ...
  case EggProgramBinary::Unequal:
    if (left.hasAny(egg::ovum::ValueFlags::AnyQ)) {
      ...
      auto equals = egg::ovum::Value::equals(left, right, egg::ovum::ValueCompare::PromoteInts);
      return egg::ovum::Value(!equals);
    }
...
bool egg::yolk::EggProgramContext::operand(egg::ovum::Value& dst, const IEggProgramNode& src, egg::ovum::ValueFlags expected, const char* expectation) {
...
egg::ovum::Value egg::yolk::EggProgramContext::coalesceNull(const egg::ovum::Value& left, egg::ovum::Value& right, const IEggProgramNode& rhs) {
  if (!left.isNull()) {
    // Short-circuit
    right = egg::ovum::Value::Void;
    return left;
  }
  right = rhs.execute(*this);
  return right;
...
egg::ovum::Value egg::yolk::EggProgramContext::logicalBool(const egg::ovum::Value& left, egg::ovum::Value& right, const IEggProgramNode& rhs, const char* operation, EggProgramBinary binary) {
  if (!left.isBool()) {
...
egg::ovum::Value egg::yolk::EggProgramContext::arithmeticBool(const egg::ovum::Value& left, egg::ovum::Value& right, const IEggProgramNode& rhs, const char* operation, ArithmeticBool bools) {
  if (!left.isBool()) {
    return this->unexpected("Expected left-hand side of " + std::string(operation) + " to be 'bool'", left);
  }
  right = rhs.execute(*this);
...
egg::ovum::Value egg::yolk::EggProgramContext::arithmeticInt(const egg::ovum::Value& left, egg::ovum::Value& right, const IEggProgramNode& rhs, const char* operation, ArithmeticInt ints) {
  if (!left.isInt()) {
...
egg::ovum::Value egg::yolk::EggProgramContext::arithmeticBoolInt(const egg::ovum::Value& left, egg::ovum::Value& right, const IEggProgramNode& rhs, const char* operation, ArithmeticBool bools, ArithmeticInt ints) {
  if (left.isBool()) {
...
egg::ovum::Value egg::yolk::EggProgramContext::arithmeticIntFloat(const egg::ovum::Value& left, egg::ovum::Value& right, const IEggProgramNode& rhs, const char* operation, ArithmeticInt ints, ArithmeticFloat floats) {
...
egg::ovum::Value egg::yolk::EggProgramContext::call(const egg::ovum::Value& callee, const egg::ovum::IParameters& parameters) {
  auto& direct = callee;
  if (!direct.hasObject()) {
...
egg::ovum::Value egg::yolk::EggProgramContext::dotGet(const egg::ovum::Value& instance, const egg::ovum::String& property) {
