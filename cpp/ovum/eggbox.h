namespace egg::ovum {
  ...
  public:
    ...
    virtual std::shared_ptr<IEggboxFileEntry> getFileEntry(const std::string& subpath) = 0;
  };

  class IEggboxChain : public IEggbox {
  public:
    // Interface
    virtual IEggboxChain& with(const std::shared_ptr<IEggbox>& subbox) = 0;
  };
  ...
  public:
    ...
    static std::shared_ptr<IEggboxChain> createChain();
    static std::shared_ptr<IEggboxChain> createDefault();
  };
}
